...
function gameIsThemeId(value) {
    return value === "arcade" || value === "blueprint" || value === "nightshift";
}
function gameIsAchievementId(value) {
    return (value === "games_page" ||
        value === "brand_tapper" ||
...
function gameApplyTheme(themeId) {
    if (!document.body) {
        return;
    }
    if (themeId === "default") {
...
function gameShowToast(message) {
    if (!document.body) {
        return;
    }
    if (!gameToastHost) {
...
function gameUnlockAchievement(achievementId) {
...
function gameUnlockTheme(themeId) {
    const state = gameReadState();
...
function gameSetActiveTheme(themeId) {
    const state = gameReadState();
...
function gameHandleKonamiInput(event) {
...
function gameInitializeBrandTapper() {
...
function gameRenderPage() {
    ...
    if (summary) {
      ...
      <div class="games-summary-card">
        ...
        <strong>${state.activeTheme === "default" ? "Default Ops" : ((_a = gameThemes.find((theme) => theme.id === state.activeTheme)) === null || _a === void 0 ? void 0 : _a.name) || "Default Ops"}</strong>
      </div>
    `;
    }
    if (themesHost) {
...
function gameInitializePageBindings() {
...
function gameTrackPageAchievements() {
