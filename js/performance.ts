type PerformanceVisitorApiResponse = {
  ip?: string;
  org?: string;
};

type PerformanceHealthResponse = {
  mode?: string;
  service?: string;
  status?: string;
  timestamp?: string;
...
function setWidth(id: string, percent: number): void {
  const element = document.getElementById(id);
...
function setState(id: string, state: string, text: string): void {
  const element = document.getElementById(id);
  if (!element) {
    return;
  }
...
function formatPerformanceBytes(bytes: number): string {
  if (!bytes || bytes < 0) {
    return "0 KB";
  }
...
function supportsObserverType(type: string): boolean {
...
function getLegacyDomInteractive(): number | null {
...
function updateServerStatus(message: string, state: "live" | "static" | "down"): void {
...
async function getData(): Promise<void> {
  try {
    ...
    const data = (await response.json()) as PerformanceHealthResponse;
...
function updateAssetMetrics(): void {
  ...
  setText(
    "largestAsset",
    largestAsset ? `${largestAsset.shortName} (${formatPerformanceBytes(largestAsset.weight)})` : "No assets recorded"
  );
...
function updateLoadMetrics(): void {
  ...
  setText(
    "pageLoadTime",
    performanceState.pageLoadTime !== null ? formatMilliseconds(performanceState.pageLoadTime) : "Performance data not available"
  );
...
function updateOverallSummary(): void {
...
function scoreLowerIsBetter(value: number, goodThreshold: number, poorThreshold: number): number {
...
function startAnimationBudgetSample(): void {
  ...
  const sampleFrame = (currentTime: number) => {
    ...
    if (averageFrame === null) {
      ...
      setText(
        "animationBudgetNote",
        `Average frame time stayed near ${averageFrame.toFixed(1)} ms with only ${(overBudgetRatio * 100).toFixed(0)}% of frames over budget.`
      );
    } else if (overBudgetRatio <= 0.25) {
      setText("animationBudgetStatus", "Tight");
      setText(
        "animationBudgetNote",
        `Frame pacing is acceptable but ${(overBudgetRatio * 100).toFixed(0)}% of sampled frames exceeded 16.7 ms.`
      );
    } else {
      setText("animationBudgetStatus", "Janky");
      setText(
        "animationBudgetNote",
        `Rendering pressure is visible: ${(overBudgetRatio * 100).toFixed(0)}% of sampled frames missed the 16.7 ms budget.`
      );
    }
...
function populateVisitorInfo(): void {
  ...
  const navigationEntry = getNavigationEntry();
  const networkType = navigator.connection?.effectiveType || navigator.connection?.type || "Not available in this browser";
  ...
  void fetch("https://ipapi.co/json/", { cache: "no-store" })
    .then((response) => {
      ...
      return response.json() as Promise<PerformanceVisitorApiResponse>;
    })
    .then((data) => {
...
function initializePerformanceDashboard(): void {
  ...
  observeEntryType("first-input", (entries) => {
    const firstInputEntry = entries[0] as FirstInputLike | undefined;
